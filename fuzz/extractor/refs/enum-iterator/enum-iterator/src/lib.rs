...
macro_rules! impl_sequence_for_int_single {
    ($ty:ty) => {
        impl Sequence for $ty {
...
impl_sequence_for_int_single!(i8);
impl_sequence_for_int_single!(u8);
impl_sequence_for_int_single!(i16);
impl_sequence_for_int_single!(u16);
impl_sequence_for_int_single!(i32);
impl_sequence_for_int_single!(u32);
impl_sequence_for_int_single!(i64);
impl_sequence_for_int_single!(u64);
impl_sequence_for_int_single!(i128);
impl_sequence_for_int_single!(u128);
impl_sequence_for_int_single!(f32);
impl_sequence_for_int_single!(f64);
...
impl<T: Sequence> Sequence for Vec<T> {
    const CARDINALITY: usize = T::CARDINALITY;

    fn next(&self) -> Option<Self> {
        let value = self.first()?;
        match T::next(value) {
            None => None,
            Some(next_val) => Some(vec![next_val]),
        }
    }

    fn previous(&self) -> Option<Self> {
        let value = self.first()?;
        match T::previous(value) {
            None => None,
            Some(next_val) => Some(vec![next_val]),
        }
    }
