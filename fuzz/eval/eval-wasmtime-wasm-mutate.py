...
# This code evaluates wasmtime compile, which uses wasm-mutate
...
def run_cargo_fuzz():
    ...
    args = [cargo_path, "fuzz", "run", "--release", "compile"]
...
def cov_cargo_fuzz():
    ...
    args = [cargo_path, "fuzz", "coverage", "--release", "compile", "--", "-rss_limit_mb=0"]
    cwd = os.path.join(os.path.dirname(os.path.dirname(root_dir)), "targets", "wasmtime", "wasmtime", "fuzz")
    subp = subprocess.Popen(
...
def build_cargo_fuzz():
    ...
    args = [cargo_path, "fuzz", "build", "--release", "compile"]
    cwd = os.path.join(os.path.dirname(os.path.dirname(root_dir)), "targets", "wasmtime", "wasmtime", "fuzz")
    subp = subprocess.Popen(
        args, env=env, shell=False, cwd=cwd,
        stdout=subprocess.DEVNULL, stderr=subprocess.PIPE
...
if __name__ == "__main__":
    ...
    corpus_dir = os.path.join(root, "targets", "wasmtime", "wasmtime", "fuzz", "corpus", "compile")
    coverage_dir = os.path.join(root, "targets", "wasmtime", "wasmtime", "fuzz", "coverage", "compile")
