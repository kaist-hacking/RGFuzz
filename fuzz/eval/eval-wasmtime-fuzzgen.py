...
# This code evaluates wasmtime fuzzgen
...
def run_cargo_fuzz():
    ...
    args = [cargo_path, "fuzz", "run", "--release", "cranelift-fuzzgen"]
...
def cov_cargo_fuzz():
    ...
    args = [cargo_path, "fuzz", "coverage", "--release", "cranelift-fuzzgen", "--", "-rss_limit_mb=0"]
    cwd = os.path.join(os.path.dirname(os.path.dirname(root_dir)), "targets", "wasmtime", "wasmtime", "fuzz")
    subp = subprocess.Popen(
...
def build_cargo_fuzz():
    ...
    args = [cargo_path, "fuzz", "build", "--release", "cranelift-fuzzgen"]
    cwd = os.path.join(os.path.dirname(os.path.dirname(root_dir)), "targets", "wasmtime", "wasmtime", "fuzz")
    subp = subprocess.Popen(
        args, env=env, shell=False, cwd=cwd,
        stdout=subprocess.DEVNULL, stderr=subprocess.PIPE
...
if __name__ == "__main__":
    ...
    corpus_dir = os.path.join(root, "targets", "wasmtime", "wasmtime", "fuzz", "corpus", "cranelift-fuzzgen")
    coverage_dir = os.path.join(root, "targets", "wasmtime", "wasmtime", "fuzz", "coverage", "cranelift-fuzzgen")
