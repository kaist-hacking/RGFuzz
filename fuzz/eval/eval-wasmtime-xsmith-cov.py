...
if __name__ == "__main__":
    assert config.engine_name == "wasmtime-xsmith"
    ...
    # profraw merge
    cur_cwd = os.getcwd()
    os.chdir(config.xsmith_wasmtime_path)
    for timeset in os.listdir(backup_dir):
        process_dir = os.path.join(backup_dir, timeset)
        arch_dirs = map(lambda x: (x, os.path.join(process_dir, x)), config.wasmtime_xsmith_arch_list)
        ...
        for arch, arch_dir in arch_dirs:
            ...
            if arch == "x64":
                compilation_dir = config.xsmith_wasmtime_path
                binary_name = os.path.join(
                    config.xsmith_wasmtime_path,
                    "target", "release", "load-wasmtime"
                )
            else:
                compilation_dir = config.xsmith_wasmtime_path
                binary_name = os.path.join(
                    config.xsmith_wasmtime_path,
                    "target", arch_ref_name, "release", "load-wasmtime"
                )
