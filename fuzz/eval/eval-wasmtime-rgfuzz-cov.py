...
if __name__ == "__main__":
    ...
    # os.chdir(os.path.join(root, "targets", "wasmtime"))
    ...
    # profraw merge
    cur_cwd = os.getcwd()
    os.chdir(os.path.join(root, "targets", "wasmtime", "wasmtime-wrapper"))
        ...
        for arch, arch_dir in arch_dirs:
            ...
            if arch == "x64":
                compilation_dir = os.path.join(
                    root, "targets", "wasmtime", "wasmtime-wrapper"
                )
                binary_name = os.path.join(
                    root, "targets", "wasmtime", "wasmtime-wrapper", 
                    "target", "release", "wasmtime-wrapper"
                )
            else:
                compilation_dir = os.path.join(
                    root, "targets", "wasmtime", "wasmtime-wrapper"
                ...
                binary_name = os.path.join(
                    root, "targets", "wasmtime", "wasmtime-wrapper", 
                    "target", arch_ref_name, "release", "wasmtime-wrapper"
                )
