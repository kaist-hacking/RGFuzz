...
if __name__ == "__main__":
    ...
    # os.chdir(os.path.join(root, "targets", "wasmer"))
    ...
    # profraw merge
    cur_cwd = os.getcwd()
    os.chdir(os.path.join(root, "targets", "wasmer", "wasmer-wrapper"))
    for timeset in os.listdir(backup_dir):
        process_dir = os.path.join(backup_dir, timeset)
        arch_dirs = map(lambda x: (x, os.path.join(process_dir, x)), config.wasmer_arch_list)
        ...
        for arch, arch_dir in arch_dirs:
            ...
            if arch == "x64":
                compilation_dir = os.path.join(
                    root, "targets", "wasmer", "wasmer-wrapper"
                )
                binary_name = os.path.join(
                    root, "targets", "wasmer", "wasmer-wrapper", 
                    "target", "release", "wasmer-wrapper"
                )
            else:
                compilation_dir = os.path.join(
                    root, "targets", "wasmer", "wasmer-wrapper"
                ...
                binary_name = os.path.join(
                    root, "targets", "wasmer", "wasmer-wrapper", 
                    "target", arch_ref_name, "release", "wasmer-wrapper"
                )
