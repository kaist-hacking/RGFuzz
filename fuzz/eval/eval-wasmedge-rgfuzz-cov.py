...
if __name__ == "__main__":
    ...
    # os.chdir(os.path.join(root, "targets", "wasmedge"))
    # os.system("./build-cov.sh")
    # os.chdir(os.path.join(root, "fuzz", "extractor"))
    ...
    # profraw merge
    cur_cwd = os.getcwd()
    os.chdir(os.path.join(root, "targets", "wasmedge", "wasmedge-wrapper"))
    for timeset in os.listdir(backup_dir):
        process_dir = os.path.join(backup_dir, timeset)
        arch_dirs = map(lambda x: (x, os.path.join(process_dir, x)), config.wasmedge_arch_list)
        ...
        for arch, arch_dir in arch_dirs:
            ...
            # change later following the architectures
            compilation_dir = os.path.join(
                root, "targets", "wasmedge", "WasmEdge"
            )
            binary_name = os.path.join(
                root, "targets", "wasmedge", "WasmEdge", 
                "build", "lib", "api", "libwasmedge.so.0"
            )
