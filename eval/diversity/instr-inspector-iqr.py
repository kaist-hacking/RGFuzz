...
def add_to_plot(path, ax):
    ...
    # iqr
    q1 = instr_counts[int(len(instr_counts) * 0.25)][1]
    q3 = instr_counts[int(len(instr_counts) * 0.75)][1]
    instr_counts = [x for x in instr_counts if q1 <= x[1] <= q3]
...
if args.cumulative:
    ax.set_title("Instructions - IQR")
    ax.set_ylabel("Cumulative Frequency")
    fig.savefig(os.path.join(coverage_root, "cumul-iqr.png"), bbox_inches = 'tight')
    fig.savefig(os.path.join(coverage_root, "cumul-iqr.pdf"), bbox_inches = 'tight')
else:
    ax.set_title("Instructions - IQR")
    ax.set_ylabel("Frequency")
    fig.savefig(os.path.join(coverage_root, "instr-iqr.png"), bbox_inches = 'tight')
    fig.savefig(os.path.join(coverage_root, "instr-iqr.pdf"), bbox_inches = 'tight')